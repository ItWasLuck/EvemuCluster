...
#ifdef WIN32
#ifdef ASCENT_FRAMEWORK_ENABLE_CRASH_HANDLER
...
DWORD mini_dump_type = MiniDumpWithIndirectlyReferencedMemory;
...
void SetMinidumpType(DWORD type)
{
	printf("Minidump type: 0x%.8X -> 0x%.8X\n", mini_dump_type, type);
	mini_dump_type = type;
}
...
int __cdecl HandleCrash(PEXCEPTION_POINTERS pExceptPtrs)
{
	...
	else
	{
		...
		MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(),
			hDump, (MINIDUMP_TYPE)mini_dump_type, &info, 0, 0);

		CloseHandle(hDump);
	}
	SetPriorityClass(GetCurrentProcess(), BELOW_NORMAL_PRIORITY_CLASS);
	OnCrash(!IsDebuggerPresent());	  

	return EXCEPTION_CONTINUE_SEARCH;
}

#endif//ASCENT_FRAMEWORK_ENABLE_CRASH_HANDLER
#endif//WIN32
