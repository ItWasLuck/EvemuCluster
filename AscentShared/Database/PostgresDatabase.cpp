...
bool PostgresDatabase::Initialize(const string& _Hostname, unsigned int port, const string& _Username, const string& _Password, const string& _DatabaseName, uint32 _ConnectionCount, uint32 _BufferSize)
{
	return Initialize( _Hostname.c_str(), port, _Username.c_str(), _Password.c_str(), _DatabaseName.c_str(),_ConnectionCount, _BufferSize );
}
...
{
	Log.Notice("PostgresDatabase", "Initializing");
	uint32 i;
	...
	for( i = 0; i < ConnectionCount; ++i )
	{
		...
		if ( PQstatus( con->PgSql ) == CONNECTION_BAD )
		{
			Log.Error("PostgresDatabase", "Connection failed due to: `%s`", PQerrorMessage( con->PgSql ) );
			PQfinish( con->PgSql );
			return false;
	...
	Log.Success("PostgresDatabase", "Connected");
...
{
	Log.Notice("PostgresDatabase", "Shutting down...");
	int32 i;
	PostgresDatabaseConnection * con;
